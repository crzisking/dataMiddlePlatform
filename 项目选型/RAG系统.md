...
#### 上传限制（已确定）
- **格式白名单**：实际只收 **PDF、Word（.docx）、Excel（.xlsx）、纯文本/Markdown（.txt/.md）**，其余一律拒绝（理由：RAG 只能消化可抽取文本的文件；同时挡住可执行/恶意文件）。
  - ⚠️ **老二进制 `.doc/.xls/.wps` 不支持解析，已从白名单去掉**——遇到先转 docx/xlsx 再入库（配置项 `UPLOAD_ALLOWED_EXTS=pdf,docx,xlsx,txt,md`）。白名单**可配置**，后续可加 PPT、图片等。
...
## 八、问答的引用来源（P4，已落地 + 2026-06-25 修复）
- 回答带 `sources`：本轮检索命中的文档 + **现签的 MinIO 下载链接**（链接限时、不存库，用时现签）。
- 采集靠 ContextVar 旁路（工具只能返字符串，检索时把命中文档旁路记下，端点据此拼 sources）。
- **多轮坑与修复**：早先模型若从对话历史直接答、本轮不检索，sources 会空。已修：
  - A 系统提示词强制知识类问题**每轮重新检索**；
  - B 来源**随消息存库**（`messages.sources`，只存 id+名），历史回看时现签链接还原；
  - C `conversation_id` 不传则服务端新建会话并返回（杜绝共享 "default" 历史串台）。
